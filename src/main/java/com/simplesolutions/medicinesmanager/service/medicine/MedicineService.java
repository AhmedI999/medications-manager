...
//        Optional<Medicine> patientMedicine = medicineDao.selectPatientMedicineById(patientId, medicineId);
//        return patientMedicine.orElseThrow(() -> new ResourceNotFoundException
//                ("Medicine (%s) not found".formatted(patientMedicine.get().getBrandName())));
    ...
    public boolean doesMedicineExists(String brandName){
        return medicineDao.doesPatientMedicineExists(brandName);
    }
    public void savePatientMedicine(MedicineRegistrationRequest request, Patient patient){
        if (doesMedicineExists(request.getBrandName()))
